...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, addr_t size, addr_t *alloc_addr)
{
	...
	if (caller == NULL || caller->krnl == NULL || caller->krnl->mm == NULL || caller->mm == NULL || alloc_addr == NULL)
	{
...
{
	if (mm == NULL || data == NULL || caller == NULL || caller->krnl == NULL)
		return -1;
...
addr_t __kmalloc(struct pcb_t *caller, int vmaid, int rgid, addr_t size, addr_t *alloc_addr)
{
...
#endif
	if (_syscall(krnl, caller->pid, 17, &regs) != 0)
		return (addr_t)-1;
	...
	*alloc_addr = rgnode.rg_start;
	...
	{
		int start_pgn = rgnode.rg_start / PAGING64_PAGESZ;
		int end_pgn   = (rgnode.rg_end - 1) / PAGING64_PAGESZ;
		int i;
...
{
	if (caller == NULL || caller->krnl == NULL || caller->krnl->mm == NULL || caller->krnl->mm->kcpooltbl == NULL || alloc_addr == NULL)
		return (addr_t)-1;
