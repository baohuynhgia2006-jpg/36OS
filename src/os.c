...
static void * ld_routine(void * args) {
  ...
  /* TODO init kernel page table directory */
	...
	if (os.krnl_pgd == NULL)
		exit(1);
	os.mm = calloc(1, sizeof(*os.mm));
	if (os.mm == NULL || init_mm(os.mm, NULL) != 0)
		exit(1);
	os.mram = mram;
	os.mswp = mswp;
...
static void read_config(const char * path) {
	...
	*/
	...
	for (i = 0; i < num_processes; i++) {
		char proc[100];
#ifdef MLQ_SCHED
		fscanf(file, "%lu %99s %lu\n", &ld_processes.start_time[i], proc, &ld_processes.prio[i]);
#else
		fscanf(file, "%lu %99s\n", &ld_processes.start_time[i], proc);
#endif
		ld_processes.path[i] = (char*)calloc(1, 128);
		snprintf(ld_processes.path[i], 128, "input/proc/%s", proc);
	}
}
